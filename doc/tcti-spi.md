...
The SPI TCTI can be used for TPM communication over SPI e.g. in embedded systems like the ESP32.
It uses user supplied methods for SPI and timing operations in order to be platform independent.
These methods are supplied to `Tss2_Tcti_Spi_Init` via the `TSS2_TCTI_SPI_PLATFORM` struct.

## Platform methods

Documentation on what the implementation for the platform methods should do can be found in `tss2_tcti_spi.h`.
...
{
    // Sleep the specified amount of milliseconds, but at least one tick to feed the watchdog
    TickType_t sleep_ticks = milliseconds / portTICK_PERIOD_MS;
    if (sleep_ticks == 0) {
        sleep_ticks = 1;
    }
...
TSS2_TCTI_SPI_PLATFORM create_tcti_spi_platform(spi_device_handle_t spi, gpio_num_t cs_pin)
{
...
TSS2_TCTI_CONTEXT* create_tcti_ctx()
{
    ...
    tcti_platform = create_tcti_spi_platform(spi, PIN_NUM_CS);
    ...
    rc = Tss2_Tcti_Spi_Init(NULL, &size, tcti_platform);
    ...
    rc = Tss2_Tcti_Spi_Init(tcti_ctx, &size, tcti_platform);
    if (rc != TSS2_RC_SUCCESS) {
